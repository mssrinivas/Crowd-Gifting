...
var len=0;
class OwnerShowMessages extends Component {
  /*constructor(props) {
    super(props);
    this.state = {
      messagelist : [],
 ...
 showAllMessages = (value)=> {
  ...
  const request = async () => {
    ...
    var url = 'http://localhost:4004/ownermessages'
      ...
      <div class="modal-body">
        <form>
          <div class="form-group">
            <button type="button" class="btn" onClick={()=>this.showAllMessages(localStorage.getItem("CurrentProperty"))}>Get All Messages</button>
...
componentDidMount()
{
localStorage.setItem("CurrentProperty",-1)
}
...
const request = async () => {
  console.log("ID IS ", value)
 // var url = 'http://localhost:4004/ownermessages'
 var url = 'http://13.52.55.245:4004/ownermessages'
  await this.setState({mess : ''})
  await console.log("AWAIT RESP ", this.state.mess)
  await fetch(url, {
      method: 'post',
...
export default OwnerShowMessages;
